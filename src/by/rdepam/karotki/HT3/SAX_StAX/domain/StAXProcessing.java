	...
	public static List<Category> process(XMLStreamReader reader) throws Exception{
		...
		while(reader.hasNext()){
			...
			case XMLStreamConstants.CHARACTERS:
				...
				switch(elementName){
				case PHOTO:
					if(dish!=null){ dish.setPhoto(text);}
					break;
				case NAME:
					if(dish!=null){ dish.setName(text);}
					break;
				case PORTION:
					if(dish!=null){ dish.setPortion(text);}
					break;
				case DESCRIPTION:
					if(dish!=null){ dish.setDescription(text);}
					break;
				case PRICE:
					if(dish!=null){ dish.setPrice(text);}
					break;
				default:
					break;
				}
